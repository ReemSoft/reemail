...
const DraftGhostInputSchema = InputSchema.omit({ canonical: true });

export type DraftGhostCleanupInput = z.input<typeof DraftGhostInputSchema>;

export const tombstoneDraftGhostIndexRow = createServerFn({ method: "POST" })
  .inputValidator((v: DraftGhostCleanupInput) => DraftGhostInputSchema.parse(v))
  .handler(async ({ data }): Promise<GhostCleanupResult> => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { verifyMailSessionToken } = await import("@/lib/mail-token.server");
    const { resolveFolderByCanonical, tombstoneSourceRow } =
    ...
    try {
      ...
      if (data.uidvalidity != null && folder.uidvalidity !== data.uidvalidity) {
        return {
          ok: true,
          applied: "uidvalidity-mismatch",
          countsDecremented: false,
      ...
      const outcome = await tombstoneSourceRow(supabaseAdmin, {
        accountId,
        folderId: folder.id,
      ...
      return {
        ok: true,
        applied: outcome.changed ? "tombstoned" : "already-tombstoned",
        countsDecremented: false,
      };
    } catch (e) {
      console.error("[tombstoneDraftGhostIndexRow] cleanup failed", e);
      return {
        ok: false,
        error: e instanceof Error ? e.message : "فشل تنظيف المسودة العالقة",
        code: "DRAFT_GHOST_CLEANUP_EXCEPTION",
      };
    }
  });
